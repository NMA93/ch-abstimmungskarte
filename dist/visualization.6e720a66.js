...
document.addEventListener("DOMContentLoaded", function () {
  d3.json("/cantons.json").then(function (cantons) {
    d3.csv("/referendum.csv").then(function (yesVotes) {
      ...
      var svg = container.append("svg").attr("width", width).attr("height", height).style("border", "1px solid #000");
      var swissprojection = d3.geoAlbers().center([0, 46.7]).rotate([-9, 0, 0]).parallels([40, 50]).scale(12500);
      var pathGenerator = d3.geoPath().projection(swissprojection);
      ...
      var swiss = svg.selectAll("path").data(cantons.features).enter().append("path").attr("d", function (d) {
        return pathGenerator(d);
      }).attr("stroke", "#FFFFFF").attr("stroke-width", 0.5).on("mouseenter", function (d) {
        ...
        var cantonMetaData = yesVotes.find(function (ja_anteil) {
          return ja_anteil.id == d.properties.id;
        });
